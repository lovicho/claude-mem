...
Other provider choices: your Gemini key, your OpenRouter key, or your Anthropic plan (`--provider claude`, which skips the sign-in step; local installs contact cmem.ai once, at signup, to create the sign-in link, and nothing else is sent to cmem.ai). Details: [Installation](/installation).
